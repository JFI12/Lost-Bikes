...
const fetchModelPosts = async(ModelStartAfter, postsPerLoad)=>{
    ...
    var indexCounter = 0
    const querySnapshot = await firebase.firestore().collection("users").where('lastlocation', "==", searchModeltext.toLowerCase()).limit(postsPerLoad).orderBy("createdAt", "desc").get();
...
const querySnapshot = await firebase.firestore().collection("users").where('lastlocation', "==", searchModeltext.toLowerCase()).limit(postsPerLoad).orderBy("createdAt", "desc").startAfter(ModelStartAfter).get();

...
const fetchModelPosts = async(ModelStartAfter, postsPerLoad)=>{
    ...
    var indexCounter = 0
    const querySnapshot = await firebase.firestore().collection("users").where('model', "==", searchModeltext.toLowerCase()).limit(postsPerLoad).orderBy("createdAt", "desc").get();
...
ListHeaderComponent={
  <>
    ...
    ListFooterComponent = {modelPosts.length>0?  (!lastModelPost && < ActivityIndicator size="small" color="#0000ff" />) : (<View style={{alignItems: "center"}}><Text style={{fontSize: "25"}}>No models recieved</Text></View>)}
...
export default SearchwithmodelScreen
